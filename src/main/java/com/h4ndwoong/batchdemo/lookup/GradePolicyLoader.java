...
/**
 * {@code member_d} 의 포인트 분포에서 등급 정책을 산출한다. <b>Step 당 한 번만 부른다.</b>
 *
 * <p>집계 한 번({@code SELECT MIN(point), MAX(point)})이며, 50만 행을 훑으므로 공짜는 아니다.
 ...
 * before/after 가 똑같이 1회 하고, 비교 축은 추천인 조회 하나로 좁혔다.
 *
...
public class GradePolicyLoader {
    ...
    private static final String SQL = "SELECT MIN(point) AS min_point, MAX(point) AS max_point FROM member_d";
     ...
     */
    public GradePolicyLoader(JdbcTemplate jdbcTemplate) {
     ...
     * 여기까지 오기 전에 {@code TableSeededValidator} 가 Job 을 세우므로, 이 예외는 실제로는
     * 그 방어가 뚫렸을 때만 보인다.
     *
     * @return 등급 정책
     * @throws IllegalStateException {@code member_d} 가 비어 있을 때
     */
    public GradePolicy load() {
        return jdbcTemplate.query(SQL, resultSet -> {
            if (!resultSet.next() || resultSet.getObject("min_point") == null) {
                throw new IllegalStateException(
                        "member_d 가 비어 있어 등급 정책을 만들 수 없다. 먼저 시딩한다.");
            }
